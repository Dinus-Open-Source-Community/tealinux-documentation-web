...
<template>
    ...
    <div class="grid grid-cols-12 md:min-h-screen bg-[#D9D9D9] py-10">
        <div class="col-span-2 md:grid hidden text-left pl-9"><DocsSidebar/></div>
        ...
        <div class="md:block hidden col-span-2">
            <h1 v-for="(item, index) in post.body.toc.links" :key="index" class="my-2">
                <a class="before:content-[w-10 border-l-[1px] py-2 pl-5 text-[#4A4A4A] border-[#858585] ] " href="#"+item.id>{{ item.text }}</a>
            </h1>
        </div>
    </div>
</template>
